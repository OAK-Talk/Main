...
public class LoginController {
	...
	@FXML
	private TextField LoginId; // 로그인 아이디

	@FXML
	private TextField LoginPw; // 로그인 비번
	...
	void cl_BackToLogin(MouseEvent event) throws IOException {
		sgt.nextScene2(event, "Login.fxml"); // 뒤로가기 누를 시 다시 로그인 페이지로
		chId = false;
	}
	...
	@FXML
	void cl_Login(MouseEvent event) throws IOException { // 로그인 클릭시
		String id = LoginId.getText(); // 로그인 아이디
		String pw = LoginPw.getText(); // 로그인 비번

		if (!id.isEmpty() && !pw.isEmpty()) {
			sgt.nextScene2(event, "FriendsList.fxml");
		} else {
			image = new Image(getClass().getResource("/image/로그인 불가.png").toExternalForm());
			notInput.setImage(image);
			failCreating(notInput);
		}
	}
	...
	@FXML
	void cl_Signup(MouseEvent event) throws IOException { // 회원가입 클릭시
		sgt.nextScene2(event, "SignUp.fxml");
	}
